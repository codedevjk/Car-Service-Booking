...
public class BookingService {
    ...
    // No @CircuitBreaker here — each sub-call is individually guarded by try/catch.
    // A circuit breaker at this level would cause the entire method to fail silently
    // if the breaker trips from previous errors (e.g., Unauthorized), hiding the real problem.
    public DashboardSummaryDTO getDashboardSummary(String customerId, boolean isAdmin) {
        ...
        if (isAdmin) {
            ...
            headers.set("X-User-Role", "ADMIN"); // Required by user-service /count endpoint
        } else {
        ...
        if (isAdmin) {
            ...
            try {
                ...
                log.info("[Dashboard] totalCustomers fetched: {}", summary.getTotalCustomers());
            } catch (Exception e) {
                log.error("[Dashboard] Failed to fetch totalCustomers from user-service: {}", e.getMessage(), e);
            }
            ...
            try {
                ...
                log.info("[Dashboard] totalCategories fetched: {}", summary.getTotalCategories());
            } catch (Exception e) {
                log.error("[Dashboard] Failed to fetch totalCategories from catalog-service: {}", e.getMessage(), e);
            }
            ...
            try {
                ...
                log.info("[Dashboard] totalServices fetched: {}", summary.getTotalServices());
            } catch (Exception e) {
                log.error("[Dashboard] Failed to fetch totalServices from catalog-service: {}", e.getMessage(), e);
            }
            ...
            try {
                ...
                log.error("[Dashboard] Failed to fetch totalVehicles from user-service: {}", e.getMessage(), e);
            }
        }
    ...
    // Kept as a safety-net fallback (referenced in case annotation is re-added)
    public DashboardSummaryDTO getDashboardSummaryFallback(String customerId, boolean isAdmin, Throwable t) {
        log.error("[Dashboard] Circuit breaker fallback triggered: {}", t.getMessage());
        return new DashboardSummaryDTO(); // return empty DTO instead of throwing
    }
